 ...
 * the structures and constants are required by the using project.
 *
 * $Rev$
 * $Date$
 *
...
const
  ...
  // Flags for all versions of IE, at least from IE4
  DOCHOSTUIFLAG_DIALOG                      = $00000001;
    {Disables selection of text in browser control}
  ...
  DOCHOSTUIFLAG_OVERRIDEBEHAVIORFACTORY     = $00000400;
    ...
    even for behaviors developed in scripts (HTML Components (HTCs))}

  // Following flags require IE5 and later
  DOCHOSTUIFLAG_CODEPAGELINKEDFONTS         = $00000800;
    {Provides font selection compatibility for Outlook Express (OE4.0 or later).
  ...
  DOCHOSTUIFLAG_URL_ENCODING_DISABLE_UTF8   = $00001000;
    {Controls how non-native URLs are transmitted over the Internet. Non-native
    refers to characters outside the multibyte encoding of the URL. If this flag
    is set, the URL is not submitted to the server in UTF-8 encoding}
  DOCHOSTUIFLAG_URL_ENCODING_ENABLE_UTF8    = $00002000;
    {Controls how non-native URLs are transmitted over the Internet. Non-native
    refers to characters outside the multibyte encoding of the URL. If this flag
    is set, the URL is submitted to the server in UTF-8 encoding}
  DOCHOSTUIFLAG_ENABLE_FORMS_AUTOCOMPLETE   = $00004000;
    {This flag enables the AutoComplete feature for forms in the
  ...
  DOCHOSTUIFLAG_ENABLE_INPLACE_NAVIGATION   = $00010000;
    {This flag enables the host to specify that navigation should happen in
    place. This means that applications hosting MSHTML directly can specify that
    navigation happen in the application's window. For instance, if this flag is
    set, you can click a link in HTML mail and navigate in the mail instead of
    opening a new IE window}
  DOCHOSTUIFLAG_IME_ENABLE_RECONVERSION     = $00020000;
    {During initialization, the host can set this flag to enable Input Method
    Editor (IME) reconversion, allowing computer users to employ IME
    reconversion while browsing Web pages}

  // Following flags require IE6 and later
  ...
  DOCHOSTUIFLAG_NOTHEME                     = $00080000;
    {Specifies that the hosted browser should not use themes for pages it
    displays}
  DOCHOSTUIFLAG_NOPICS                      = $00100000;
    {Disables PICS ratings for the hosted browser}
  DOCHOSTUIFLAG_NO3DOUTERBORDER             = $00200000;
    {Turns off any 3-D border on the outermost frame or frameset only. To turn
    borders off on all frame sets, use DOCHOSTUIFLAG_NO3DBORDER}

  // Following flags are also defined, but values are in error:
  {
  ...
  //
  // Values that that indicate the type of user interface. Used in the
...
type
  ...
  {
  TDocHostUIInfo, PDocHostUIInfo:
    Record and record pointer used by the IDocHostUIHandler.GetHostInfo method
  ...
  IDocHostUIHandler = interface(IUnknown)
    ...
    function ShowContextMenu(const dwID: DWORD; const ppt: PPOINT;
      ...
      {Called by MSHTML to display a shortcut menu.
        @param dwID [in] Specifies identifier of the shortcut menu to be
          displayed. This identifier is a bitwise shift of the value $1 by the
          shortcut menu values (e.g., CONTEXT_MENU_DEFAULT) defined in
          Mshtmhst.h. Values are:
            $01 (0x1 shl CONTEXT_MENU_DEFAULT)
            $02 (0x1 shl CONTEXT_MENU_CONTROL)
            $04 (0x1 shl CONTEXT_MENU_TABLE)
            $08 (0x1 shl CONTEXT_MENU_TEXTSELECT)
            $10 (0x1 shl CONTEXT_MENU_ANCHOR)
            $20 (0x1 shl CONTEXT_MENU_UNKNOWN).
        ...
        @return S_OK if host displayed its own user interface (e.g. a menu) -
          MSHTML will not attempt to display its own menu; S_FALSE if host did
          not display any UI - MSHTML will display its own menu.
      }
    function GetHostInfo(var pInfo: TDocHostUIInfo): HResult; stdcall;
      {Called by MSHTML to retrieve the user interface (UI) capabilities and
      requirements of the application that is hosting MSHTML. Various aspects of
    ...
    function GetOptionKeyPath(var pchKey: POLESTR;
      const dwReserved: DWORD ): HResult;
      stdcall;
      ...
      Control reverts to the default Internet Explorer registry settings.
        @param pchKey [in/out] POLESTR that receives the registry subkey string
          where the host stores its registry settings: may be changed by the
          implementor.
        @param dwReserved [in] Reserved. Always 0.
        @return S_OK if successful and pchKey has been modified, S_FALSE to use
          default registry setting, or an error value otherwise.
      }
    function GetDropTarget(const pDropTarget: IDropTarget;
      ...
      supply an alternative IDropTarget interface.
        ...
        @return S_OK if successful and a drop target has been provided, or an
    ...
    function GetExternal(out ppDispatch: IDispatch): HResult; stdcall;
      ...
      host. These methods are accessible from scripts run in the browser
      control.
  ...
  ICustomDoc = interface(IUnknown)
    ...
    function SetUIHandler(const pUIHandler: IDocHostUIHandler): HResult;
      ...
      pUIHandler's _AddRef method.
        ...
        @return S_OK on success or an error value on failure.
      }
  end;
